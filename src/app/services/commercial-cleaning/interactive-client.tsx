...
import {
  Search, ArrowRight, Sparkles, Target, Briefcase, Hotel, UtensilsCrossed,
  Store, Heart, GraduationCap, FileText, Moon, AlertCircle, Zap, BadgeCheck
} from "lucide-react"
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div className="relative flex-1">
            ...
            <Input
              type="text"
              placeholder="Search commercial services..."
              className="pl-10 h-12 border-2 border-gray-200 focus:border-slate-500"
              value={searchTerm}
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0">
            {categories.map(category => (
              <Button
                ...
                className={`whitespace-nowrap ${activeFilter === category ? 'bg-slate-600 text-white' : 'bg-white text-gray-700 hover:bg-gray-50'}`}
              >
      ...
      <div className="text-center text-gray-600">
        Showing {filteredSubtopics.length} of {subtopics.length} services
      </div>
        ...
        {filteredSubtopics.map((subtopic, index) => {
          ...
          return (
            <Link href={subtopic.url} key={index}>
              <Card className="group h-full hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer border-2 border-gray-100 hover:border-slate-300">
                <CardHeader>
                  <div className="flex items-start justify-between mb-4">
                    <div className="w-14 h-14 bg-gradient-to-br from-slate-600 to-slate-700 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
